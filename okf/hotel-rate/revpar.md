...
# Distinctions
- **RevPAR** vs [ADR](/hotel-rate/adr.md) — RevPAR uses available rooms as the denominator and blends in occupancy; ADR uses only sold rooms and isolates price.
- **RevPAR** vs [Occupancy](/hotel-rate/occupancy.md) — Occupancy rate is one of the two inputs to RevPAR (RevPAR = ADR x occupancy rate); on its own it ignores the rate achieved.
