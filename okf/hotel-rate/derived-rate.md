...
- [Open Pricing](/hotel-rate/open-pricing.md) — contrasts

# Distinctions
- **Derived Rate** vs [Rate Plan](/hotel-rate/rate-plan.md) — Every derived rate is a rate plan, but a derived rate is specifically one whose values are computed from a parent rate; a standalone (master) rate plan holds its own independently maintained values.
- **Derived Rate** vs [BAR](/hotel-rate/bar.md) — BAR is typically the parent rate from which derived rates are calculated; a derived rate is a child like 'BAR minus 10%, non-refundable'. BAR is the source value, the derived rate is the dependent value.
- **Derived Rate** vs [Package Rate](/hotel-rate/package-rate.md) — A package rate bundles room with extras (meals, parking) and may itself be derived from a base room rate; a derived rate refers narrowly to the price-linkage mechanism, regardless of whether any package components are bundled.
