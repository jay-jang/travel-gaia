...
providerTerms:
  ...
  - provider: NDC/IATA
    ...
    context: 'NDC carries the O&D pair as the OriginDestination element in AirShopping requests/responses, the unit against which offers are shopped and priced.'
    context_ko: 'NDC는 O&D 쌍을 AirShopping 요청/응답의 OriginDestination 요소로 전달하며, 이는 오퍼를 조회·가격산정하는 단위다.'
    relationship: same
relationships:
  - type: related
    targetTerm: Journey
...
| NDC/IATA | `OriginDestination` | same | NDC carries the O&D pair as the OriginDestination element in AirShopping requests/responses, the unit against which offers are shopped and priced. |
