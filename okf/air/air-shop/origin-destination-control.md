...
providerTerms:
  - provider: Sabre
    ...
    relationship: same
  - provider: PROS
    term: O&D / Network Revenue Management (DAVN-style bid-price control)
    context: 'PROS RM implements O&D control via network optimization that generates displacement costs and pseudo-fares per leg, then leg optimization that sets bid prices reflecting full itinerary value.'
    context_ko: 'PROS RM은 레그별 displacement cost와 pseudo-fare를 생성하는 네트워크 최적화와, 전체 여정 가치를 반영한 bid price를 설정하는 레그 최적화로 O&D control을 구현한다.'
    relationship: same
relationships:
  - type: broader
...
| PROS | `O&D / Network Revenue Management (DAVN-style bid-price control)` | same | PROS RM implements O&D control via network optimization that generates displacement costs and pseudo-fares per leg, then leg optimization that sets bid prices reflecting full itinerary value. |
...
# Distinctions
- **Origin & Destination Control (O&D Control)** vs [Origin & Destination](/air/air-shop/origin-destination.md) — Origin & Destination is the geographic point-pair (the market unit); O&D control is the revenue-management technique that values seats over that full O&D rather than per leg.
- **Origin & Destination Control (O&D Control)** vs [Married Segment](/air/air-ops/married-segment.md) — A married segment is the booking-side rule that ties connecting segments so they cannot be split; O&D control is the RM rationale that produces and prices those itinerary-level availabilities.
- **Origin & Destination Control (O&D Control)** vs [Virtual Nesting](/air/air-shop/virtual-nesting.md) — O&D control is the general objective of valuing seats over the whole itinerary; virtual nesting is one specific technique that achieves it by mapping O&D products into per-leg virtual buckets controlled with nested limits. Bid-price control is the alternative technique for the same objective.

# Citations
[1] [Journal of Revenue and Pricing Management (Springer) — Unlocking the value from origin and destination revenue management](https://link.springer.com/article/10.1057/s41272-017-0101-5)
[2] [MIT (Peter Belobaba) — Network Revenue Management: O&D Control](https://ocw.mit.edu/courses/16-75j-airline-management-spring-2006/)
