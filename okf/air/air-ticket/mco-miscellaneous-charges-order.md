...
providerTerms:
  ...
  - provider: NDC/IATA
    term: EMD-S (MCO replacement)
    context: 'IATA replaced the paper MCO with the electronic EMD; the Standalone EMD (EMD-S) is the modern accountable document used to collect non-flight charges, residual value, and deposits that the MCO formerly recorded.'
    context_ko: 'IATA는 종이 MCO를 전자 EMD로 대체했으며, Standalone EMD(EMD-S)가 MCO가 기록하던 비(非)비행 요금·잔여가치·예치금을 징수하는 현대식 회계 서류다.'
    relationship: broader
  - provider: NDC/IATA
    term: vMCO / V-MPD (BSPlink)
    context: 'Where legacy MCO/MPD documents still appear in IATA BSP processing, they are handled as virtual documents (vMCO / V-MPD) in BSPlink alongside the EMD that superseded them.'
    context_ko: 'IATA BSP 처리에서 기존 MCO/MPD 서류가 남아 있는 경우 BSPlink에서 가상 서류(vMCO / V-MPD)로 처리되며, 이를 대체한 EMD와 함께 관리된다.'
...
| NDC/IATA | `EMD-S (MCO replacement)` | broader | IATA replaced the paper MCO with the electronic EMD; the Standalone EMD (EMD-S) is the modern accountable document used to collect non-flight charges, residual value, and deposits that the MCO formerly recorded. |
| NDC/IATA | `vMCO / V-MPD (BSPlink)` | related | Where legacy MCO/MPD documents still appear in IATA BSP processing, they are handled as virtual documents (vMCO / V-MPD) in BSPlink alongside the EMD that superseded them. |
...
# Distinctions
- **MCO (Miscellaneous Charges Order)** vs [EMD](/air/air-ticket/emd.md) — The EMD is the electronic successor to the MCO; the MCO was the paper document, while the EMD is the IATA-standard electronic equivalent (EMD-S / EMD-A) that has largely replaced it.
