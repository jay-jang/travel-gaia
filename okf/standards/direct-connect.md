...
# Distinctions
- **Direct Connect** vs [GDS](/standards/gds.md) — A Direct Connect is a point-to-point link straight to one supplier; a GDS is a shared intermediary aggregating many suppliers for many sellers.
- **Direct Connect** vs [Aggregator](/standards/aggregator.md) — A Direct Connect is the seller's own integration to a single supplier; an aggregator offers one connection that consolidates many suppliers' direct connects on the seller's behalf.
- **Direct Connect** vs [NDC](/standards/ndc.md) — Direct Connect is a distribution arrangement (who connects to whom); NDC is a messaging standard that a Direct Connect may (but need not) use.
