---
type: Business Term
title: Tour Conductor Credit (TC)
description: 'The core group-booking incentive in cruise distribution: one complimentary berth (a ''tour conductor'' credit) earned for every fixed number of full-fare cabins booked in a group, typically one free berth per eight cabins (i.e., per 15-to-1 berth ratio). The TC credit can be taken as a free berth for an escort/host, used as additional agency profit, or applied to reduce the group''s price.'
tags:
  - cruise
  - active
timestamp: '2026-06-18T00:00:00Z'
id: tour-conductor-credit-tc
vertical: cruise
category: cruise
conceptType: business-term
status: active
abbreviation: TC
term_ko: '투어 컨덕터 크레딧 (Tour Conductor Credit, TC)'
definition_ko: '크루즈 유통에서 핵심이 되는 단체 예약 인센티브로, 단체에 정상 운임 선실을 일정 수 예약할 때마다 무료 침대 1개(투어 컨덕터 크레딧)를 적립해 주는 제도다. 일반적으로 선실 8개당 무료 침대 1개(즉 유료 15 대 무료 1 비율)이며, TC 크레딧은 인솔자/호스트용 무료 침대로 쓰거나, 여행사의 추가 수익으로 가져가거나, 단체 가격을 낮추는 데 적용할 수 있다.'
longDef: 'Tour Conductor Credits are unique to cruise (and packaged tour) group sales and underpin agency group economics. The conventional accrual is one free berth per eight full-fare cabins (two berths each = 16 berths, of which 15 are paid), scaling linearly: 16 cabins earn two berths (one free cabin), 24 cabins earn three berths, and so on. The credit''s monetary value historically equaled the predominant booked cabin category, but several lines (led by Royal Caribbean Cruises Ltd. across Royal Caribbean International, Celebrity and Azamara for 2018+ sailings) changed it to the average cruise fare of all categories booked in the group. Because the value can be monetized rather than physically occupied, TC credits function as a flexible group discount, commission supplement, or host-berth subsidy — making them the central group-pricing lever in cruise distribution with no direct equivalent in air or hotel group programs.'
longDef_ko: '투어 컨덕터 크레딧은 크루즈(및 패키지 투어) 단체 판매에 고유한 제도로 여행사 단체 수익 구조의 근간을 이룬다. 관행적 적립 기준은 정상 운임 선실 8개당 무료 침대 1개(선실당 침대 2개 = 16개 중 15개 유료)이며 선형으로 확대된다. 즉 선실 16개는 침대 2개(무료 선실 1개), 24개는 침대 3개를 적립하는 식이다. 크레딧의 금전 가치는 과거 단체 내 다수 선실 카테고리 기준이었으나, 여러 선사(Royal Caribbean Cruises Ltd.가 Royal Caribbean International·Celebrity·Azamara에서 2018년 이후 출항 단체에 적용하며 선도)가 단체에 예약된 모든 카테고리의 평균 크루즈 운임 기준으로 변경했다. 가치를 실제 점유 대신 현금화할 수 있어, TC 크레딧은 유연한 단체 할인·커미션 보전·호스트 침대 보조 수단으로 기능한다. 이는 크루즈 유통의 핵심 단체 가격 지렛대이며 항공·호텔 단체 프로그램에는 직접적 등가물이 없다.'
aliases:
  - TC
  - TC Credit
  - Tour Conductor
  - Free Berth Credit
  - Comp Berth
providerTerms:
  - provider: Royal Caribbean Cruises Ltd.
    term: Tour Conductor (TC) Credit
    context: 'For 2018+ group sailings the TC credit value is calculated on the average cruise fare of all stateroom categories booked in the group, replacing the prior predominant-category method; one TC is earned per eight cabins.'
    context_ko: '2018년 이후 단체 출항부터 TC 크레딧 가치는 단체에 예약된 전 선실 카테고리의 평균 크루즈 운임으로 산정하며, 이는 종전 다수 카테고리 방식을 대체한다. 선실 8개당 TC 1개를 적립한다.'
    relationship: same
  - provider: Carnival Cruise Line
    term: Tour Conductor credit / free berth (1 per 15) + Group Amenity Points
    context: 'Carnival awards one free berth (tour conductor) per 15 full-fare lower berths — i.e. the 16th guest sails free — calculated on the group''s average fare, and lets Group Amenity Points accelerate the ratio or convert to perks/OBC.'
    context_ko: '카니발은 정상 운임 로어버스 15개당 무료 버스(투어 컨덕터) 1개를 부여해 16번째 승객이 무료 탑승하며, 그룹 평균 운임으로 계산하고 Group Amenity Points로 비율을 가속하거나 혜택·OBC로 전환할 수 있다.'
    relationship: same
relationships:
  - type: parent
    targetTerm: Group PNR
  - type: related
    targetTerm: Cabin Category
  - type: related
    targetTerm: Cruise-Only Fare
  - type: related
    targetTerm: Non-Commissionable Fare (NCF)
  - type: related
    targetTerm: Single Supplement
distinctions:
  - targetTerm: Onboard Credit (OBC)
    explanation: 'An OBC is a spending credit redeemable only against onboard purchases for an individual booking; a TC credit is a group-level free-berth entitlement earned by booking volume that can be taken as a comp berth, agency profit, or a reduction in the group fare. OBC rewards a guest''s onboard spend potential; TC rewards the agent''s group production.'
    explanation_ko: 'OBC는 개별 예약 건에 대해 선상 구매에만 사용할 수 있는 지출 크레딧인 반면, TC 크레딧은 예약 물량으로 적립되는 단체 단위 무료 침대 권리로 무료 침대·여행사 수익·단체 운임 인하로 활용할 수 있다. OBC는 승객의 선상 지출 잠재력에 대한 보상이고, TC는 여행사의 단체 생산 실적에 대한 보상이다.'
  - targetTerm: Single Supplement
    explanation: A Single Supplement is a surcharge a solo guest pays to cover the unsold second berth in a double-occupancy cabin; a TC credit is the inverse incentive — the line gives back the value of one berth per eight cabins to reward group volume rather than charging for an empty one.
    explanation_ko: 'Single Supplement은 2인 1실 선실에서 팔리지 않은 둘째 침대를 메우기 위해 1인 승객이 내는 할증이고, TC 크레딧은 그 반대 인센티브다. 빈 침대에 요금을 매기는 대신, 선사가 단체 물량을 보상하기 위해 선실 8개당 침대 1개의 가치를 돌려준다.'
sources:
  - org: Travel Agent Central
    name: Royal Caribbean's New Tour Conductor Credit Policy to Use Average Cruise Fare
    version: ''
    section: ''
    url: 'https://www.travelagentcentral.com/cruises/royal-caribbean-s-new-tour-conductor-credit-policy-to-use-average-cruise-fare'
  - org: Cruise Fever
    name: Tour Conductor Credits — one free berth per eight cabins (group free-berth ratio)
    version: ''
    section: ''
    url: 'https://cruisefever.net/1-way-to-cruise-for-free-tour-conductor/'
icon: <svg viewBox="0 0 48 48" stroke="currentColor" stroke-width="2" fill="none" stroke-linecap="round" stroke-linejoin="round"><circle cx="16" cy="15" r="4"/><path d="M9 30c0-4 3-7 7-7s7 3 7 7"/><circle cx="33" cy="15" r="4"/><path d="M26 30c0-4 3-7 7-7s7 3 7 7"/><line x1="30" y1="38" x2="42" y2="38"/><line x1="36" y1="33" x2="36" y2="43"/></svg>
---

> The core group-booking incentive in cruise distribution: one complimentary berth (a 'tour conductor' credit) earned for every fixed number of full-fare cabins booked in a group, typically one free berth per eight cabins (i.e., per 15-to-1 berth ratio). The TC credit can be taken as a free berth for an escort/host, used as additional agency profit, or applied to reduce the group's price.

Tour Conductor Credits are unique to cruise (and packaged tour) group sales and underpin agency group economics. The conventional accrual is one free berth per eight full-fare cabins (two berths each = 16 berths, of which 15 are paid), scaling linearly: 16 cabins earn two berths (one free cabin), 24 cabins earn three berths, and so on. The credit's monetary value historically equaled the predominant booked cabin category, but several lines (led by Royal Caribbean Cruises Ltd. across Royal Caribbean International, Celebrity and Azamara for 2018+ sailings) changed it to the average cruise fare of all categories booked in the group. Because the value can be monetized rather than physically occupied, TC credits function as a flexible group discount, commission supplement, or host-berth subsidy — making them the central group-pricing lever in cruise distribution with no direct equivalent in air or hotel group programs.

**한국어 / Korean** — **투어 컨덕터 크레딧 (Tour Conductor Credit, TC)** — 크루즈 유통에서 핵심이 되는 단체 예약 인센티브로, 단체에 정상 운임 선실을 일정 수 예약할 때마다 무료 침대 1개(투어 컨덕터 크레딧)를 적립해 주는 제도다. 일반적으로 선실 8개당 무료 침대 1개(즉 유료 15 대 무료 1 비율)이며, TC 크레딧은 인솔자/호스트용 무료 침대로 쓰거나, 여행사의 추가 수익으로 가져가거나, 단체 가격을 낮추는 데 적용할 수 있다.

투어 컨덕터 크레딧은 크루즈(및 패키지 투어) 단체 판매에 고유한 제도로 여행사 단체 수익 구조의 근간을 이룬다. 관행적 적립 기준은 정상 운임 선실 8개당 무료 침대 1개(선실당 침대 2개 = 16개 중 15개 유료)이며 선형으로 확대된다. 즉 선실 16개는 침대 2개(무료 선실 1개), 24개는 침대 3개를 적립하는 식이다. 크레딧의 금전 가치는 과거 단체 내 다수 선실 카테고리 기준이었으나, 여러 선사(Royal Caribbean Cruises Ltd.가 Royal Caribbean International·Celebrity·Azamara에서 2018년 이후 출항 단체에 적용하며 선도)가 단체에 예약된 모든 카테고리의 평균 크루즈 운임 기준으로 변경했다. 가치를 실제 점유 대신 현금화할 수 있어, TC 크레딧은 유연한 단체 할인·커미션 보전·호스트 침대 보조 수단으로 기능한다. 이는 크루즈 유통의 핵심 단체 가격 지렛대이며 항공·호텔 단체 프로그램에는 직접적 등가물이 없다.

**Aliases:** `TC`, `TC Credit`, `Tour Conductor`, `Free Berth Credit`, `Comp Berth`

# Provider & standard equivalents

| Provider | Term | Relationship | Context |
| --- | --- | --- | --- |
| Royal Caribbean Cruises Ltd. | `Tour Conductor (TC) Credit` | same | For 2018+ group sailings the TC credit value is calculated on the average cruise fare of all stateroom categories booked in the group, replacing the prior predominant-category method; one TC is earned per eight cabins. |
| Carnival Cruise Line | `Tour Conductor credit / free berth (1 per 15) + Group Amenity Points` | same | Carnival awards one free berth (tour conductor) per 15 full-fare lower berths — i.e. the 16th guest sails free — calculated on the group's average fare, and lets Group Amenity Points accelerate the ratio or convert to perks/OBC. |

# Related
- [Group PNR](/air/air-ops/group-pnr.md) — parent
- [Cabin Category](/cruise/cruise/cabin-category.md) — related
- [Cruise-Only Fare](/cruise/cruise/cruise-only-fare.md) — related
- [Non-Commissionable Fare (NCF)](/cruise/cruise/non-commissionable-fare-ncf.md) — related
- [Single Supplement](/cruise/cruise/single-supplement.md) — related

# Distinctions
- **Tour Conductor Credit (TC)** vs [Onboard Credit (OBC)](/cruise/cruise/onboard-credit-obc.md) — An OBC is a spending credit redeemable only against onboard purchases for an individual booking; a TC credit is a group-level free-berth entitlement earned by booking volume that can be taken as a comp berth, agency profit, or a reduction in the group fare. OBC rewards a guest's onboard spend potential; TC rewards the agent's group production.
- **Tour Conductor Credit (TC)** vs [Single Supplement](/cruise/cruise/single-supplement.md) — A Single Supplement is a surcharge a solo guest pays to cover the unsold second berth in a double-occupancy cabin; a TC credit is the inverse incentive — the line gives back the value of one berth per eight cabins to reward group volume rather than charging for an empty one.

# Citations
[1] [Travel Agent Central — Royal Caribbean's New Tour Conductor Credit Policy to Use Average Cruise Fare](https://www.travelagentcentral.com/cruises/royal-caribbean-s-new-tour-conductor-credit-policy-to-use-average-cruise-fare)
[2] [Cruise Fever — Tour Conductor Credits — one free berth per eight cabins (group free-berth ratio)](https://cruisefever.net/1-way-to-cruise-for-free-tour-conductor/)
