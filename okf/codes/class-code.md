...
# Distinctions
- **Class Code** vs [RBD](/air-shop/rbd.md) — The class code is the same single-letter value as the RBD; 'RBD' (reservation booking designator) is the formal/system term, while 'class code' or 'booking class' is the everyday term seen on segments, tickets, and displays.
- **Class Code** vs [Fare Basis Code](/air-shop/fare-basis-code.md) — The class code is one letter (the booking bucket); a fare basis code is a longer alphanumeric string whose first character is usually the class code and which encodes the full fare and its conditions.
