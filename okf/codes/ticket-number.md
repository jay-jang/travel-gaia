...
# Distinctions
- **Ticket Number** vs [PNR](/air-ops/pnr.md) — A PNR locator references the whole reservation (which may hold several passengers and tickets); a ticket number references a single passenger's transport document of 13 digits.
- **Ticket Number** vs [Airline Accounting Code](/codes/airline-accounting-code.md) — The accounting code is only the first 3 digits identifying the airline; the ticket number is the full 13-digit document number that contains it.
