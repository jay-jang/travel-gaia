...
# Distinctions
- **Cost Per Acquisition (CPA)** vs [Distribution Cost](/hotel-dist/distribution-cost.md) — Distribution cost is the total acquisition cost across all channels; CPA is one specific metasearch billing model (pay-per-stay/conversion) that contributes to — and is a way of controlling — that cost.
- **Cost Per Acquisition (CPA)** vs [Metasearch](/standards/metasearch.md) — Metasearch is the channel that aggregates and compares rates; CPA is one of the monetization/billing models (versus CPC) by which that metasearch channel charges advertisers.
