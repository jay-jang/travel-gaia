...
# Distinctions
- **Opaque Inventory** vs [Rate Parity](/hotel-rate/rate-parity.md) — Opaque inventory is a way to discount that does not breach rate parity, because the discount is detached from an identifiable, publicly listed property rate; rate parity is the consistency requirement that opaque selling is specifically designed to sidestep.
- **Opaque Inventory** vs [Bedbank](/hotel-dist/bedbank.md) — A bedbank is a B2B wholesaler that supplies net inventory to resellers; opaque inventory is a B2C selling model (often fed partly by such net/distressed stock) defined by hiding the property identity from the consumer until purchase.
