...
# Related
- [SIPP Code](/car/sipp-code.md) — sameAs
...
# Distinctions
- **ACRISS Code** vs [SIPP Code](/car/sipp-code.md) — ACRISS is the current four-character standard maintained by the ACRISS association; SIPP is the older GDS-origin name for essentially the same four-letter car code, still used colloquially (especially in North America). ACRISS expanded and now governs the value sets.
- **ACRISS Code** vs [Car Class](/car/car-class.md) — The ACRISS code is the standardized descriptor of a vehicle's attributes; a car class is the sellable inventory grouping (with rates and availability) that suppliers build on top of one or more ACRISS codes.
