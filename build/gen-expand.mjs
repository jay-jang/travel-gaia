// Generates build/expand-workflow.mjs with per-category data baked in.
...
// curated additional headwords per category (~8 each; agent may adjust to avoid dups)
const SUGGEST = {
  'air-ops':['Record Locator','Split PNR','Group PNR','Itinerary','Seat Map','Waitlist','Booking Status Code','Open Segment'],
  'air-shop':['Fare Rule','Class of Service','Revenue Management','Fare Construction','Pricing Unit','Tax (YQ/YR)','Surcharge','Continuous Pricing'],
  'air-ticket':['MCO (Miscellaneous Charges Order)','Exchange','Agency Debit Memo (ADM)','Residual Value','Conjunction Ticket','Penalty Fee','Open Ticket','Coupon Status'],
  'air-partner':['Airline Alliance','Joint Venture','Block Space Agreement','Wet Lease','Hub-and-Spoke','Through Fare','Connection','Re-accommodation'],
  'hotel-rate':['Rate Code','Package Rate','Corporate Rate','Stop Sell','Minimum Length of Stay (MLOS)','Release Period','Run of House (ROH)','Non-Refundable Rate'],
  'hotel-dist':['Hotel Switch','Property Mapping','Rate Shopping','Direct Booking','No-Show','Booking Engine','Distribution Cost','Two-Way XML'],
  'pay':['PCI DSS','Strong Customer Authentication (SCA)','Form of Payment','Remittance','Tokenization','Acquirer','Interchange Fee','Settlement Cycle'],
  'codes':['Airline Accounting Code','Flight Number','Aircraft Type Code','Office ID / IATA Number','Ticketing Prefix','Meal Code','Class Code','Station Code'],
  'customer':['Known Traveler Number (KTN)','Redress Number','Unaccompanied Minor (UMNR)','Mileage Accrual','Award Redemption','Companion','Traveler Preference','Corporate Profile'],
  'standards':['EDIFACT','REST API','Direct Connect','Dynamic Packaging','Offer Management System','Order Management System (OMS)','Airline Retailing Maturity (ARM)','Shopping API'],
};

const cats = [];
const roster = [];
for (const id of Object.keys(TITLES)) {
  const arr = JSON.parse(readFileSync(`data/cat-${id}.json`,'utf8'));
  const existing = arr.map(e => e.term);
  roster.push(...existing);
  cats.push({ id, title:TITLES[id], existing, suggest:SUGGEST[id] });
}

const DATA = JSON.stringify({ cats, roster });
...
writeFileSync('build/expand-workflow.mjs', script);
console.log('wrote build/expand-workflow.mjs');
console.log('categories:', cats.length, '| roster size:', roster.length);
console.log('per cat existing counts:', cats.map(c=>`${c.id}:${c.existing.length}`).join(' '));
