...
export function fmToEntry(fm, fallbackId, fallbackCat) {
  const e = { ...fm };
  e.term = fm.title;
...
export function loadOkf(okfDir) {
  ...
  for (const f of files) {
    ...
    const rel = relative(okfDir, f).split(sep);
    const fbCat = rel.length > 1 ? rel[0] : '';
    ...
    entries.push(fmToEntry(fm, fbId, fbCat));
  }
